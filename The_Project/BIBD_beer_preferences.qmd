...
Table 5 confirms our ANOVA results. That is every Confidence interval contains the value of zero. We recognize that the block factor is not the factor of interest here, but for the sake of the post hoc we'll calculate the CI's using Tukey's HSD as well.
...
Again, no surprise here. Zero is contained in every interval.
